...
Rule #2 is a little trickier than it seems, however: what counts as an "observation"? Is it an individual, or the individual measurements taken on an individual (weight, hindfoot length, etc.), or all the individuals caught in a given plot on a given day? A table where (for example) all the individuals caught in a given plot count as a single observation is called **wide** (it has many columns and few rows), while one where each individual has its own observation is called **long** (it has many rows and few columns).

Many researchers store data in wide format by default, because it's often easier for data entry and for human readability. However, most statistical tools (including R, and especially tidyverse) work better with long format. The `gather()` function from the tidyverse converts data from wide to long format: the `spread()` function converts from long to wide (which, as we will see, can be useful for generating human-readable summaries).
...
## reorder species 
svy_w2 <- svy_w1 %>% ungroup %>%
    mutate(species_id=reorder(factor(species_id),species_count,
                              FUN=function(x) -mean(x))) %>%
    spread(species_id,species_count,fill=0)

svy_w2 %>%     select(month:RM) %>%
    mutate(`...`="...") %>% 
    head %>%   pander
```

In order to work with this data set effectively in R, we need to convert it to long format. We want to convert all of the columns corresponding to species abbreviations into a single pair of columns, one (the **key**) containing the species abbreviation and the other (the **value**) containing the number of that species counted in the corresponding 
