...
Suppose we are given a data table with species counts per plot and day, like this:
...
useful if you have a large number of identifying columns, and it's sometimes easier to
specify what to gather than what to ignore. And if the columns are in a
...
## specify columns to include (need to know last column in this case)
surveys_wide %>%
  gather(key = species_abbrev, value = count, PB:ZL)
```
